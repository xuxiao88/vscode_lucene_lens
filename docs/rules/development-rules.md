...
- 每个版本插件必须通过 core SPI 声明自己支持的 Analyzer ID 和显示名称；查询和导出只能使用当前插件声明的 Analyzer，不得按任意类名动态加载。
- 首版 Lucene 9 插件声明 Standard、Keyword、Whitespace、Simple、CJK 和 Smart Chinese Analyzer；新增版本插件可以按其 Lucene 依赖声明实际可用的集合。
